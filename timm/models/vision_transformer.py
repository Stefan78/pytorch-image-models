...
class RecurrentVisionTransformer(nn.Module):
    """ Vision Transformer
    - modified Vision Transformer (added a feedback loop)
    """
...
@register_model
def recurrent_vit_extra_tiny(pretrained=False, **kwargs):
    """ ViT-Extra_Tiny      #for testing the new architecture on CIFAR-10
    """
    model_kwargs = dict(img_size=64, patch_size=8, in_chans=3, num_classes=10, embed_dim=72, depth=6, num_heads=6)
    model = RecurrentVisionTransformer(**model_kwargs)
    return model
